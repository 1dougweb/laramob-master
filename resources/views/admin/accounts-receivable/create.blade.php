    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('admin.accounts-receivable.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Cliente/Pessoa -->
                        <div>
                            <label for="person_id" class="block text-sm font-medium text-gray-700">{{ __('Cliente/Pessoa') }}</label>
                        ...
                        <div>
                            ...
                            <select id="category" name="category" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="rent" {{ old('category') == 'rent' ? 'selected' : '' }}>{{ __('Aluguel') }}</option>
                                <option value="sale" {{ old('category') == 'sale' ? 'selected' : '' }}>{{ __('Venda') }}</option>
                                <option value="commission" {{ old('category') == 'commission' ? 'selected' : '' }}>{{ __('Comissão') }}</option>
                                <option value="service" {{ old('category') == 'service' ? 'selected' : '' }}>{{ __('Serviço') }}</option>
                                <option value="client" {{ old('category') == 'client' ? 'selected' : '' }}>{{ __('Cliente') }}</option>
                    ...
                    <div class="mt-6 flex justify-end">
                        <a href="{{ route('admin.accounts-receivable.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                            {{ __('Cancelar') }}
                        </a>
