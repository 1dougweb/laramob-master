                    ...
                    <form action="{{ route('admin.commissions.update', $commission->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="contract_id" name="contract_id" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    @foreach($contracts as $contract)
                                        <option value="{{ $contract->id }}" {{ (old('contract_id', $commission->contract_id) == $contract->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="broker_id" name="broker_id" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    @foreach($brokers as $broker)
                                        <option value="{{ $broker->id }}" {{ (old('broker_id', $commission->broker_id) == $broker->id) ? 'selected' : '' }}>
                                            {{ $broker->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="amount" id="amount" value="{{ old('amount', $commission->amount) }}" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            </div>
                            
                            <div>
                                <label for="percentage" class="block text-sm font-medium text-gray-700 mb-1">Percentual (%) *</label>
                                <input type="number" step="0.01" min="0" max="100" name="percentage" id="percentage" value="{{ old('percentage', $commission->percentage) }}" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    <option value="pending" {{ (old('status', $commission->status) == 'pending') ? 'selected' : '' }}>Pendente</option>
                                    <option value="paid" {{ (old('status', $commission->status) == 'paid') ? 'selected' : '' }}>Pago</option>
                                </select>
                            </div>
                            
                            <div class="payment-details" id="payment-details" style="{{ $commission->status == 'paid' ? 'display: block;' : 'display: none;' }}">
                                <label for="payment_date" class="block text-sm font-medium text-gray-700 mb-1">Data de Pagamento</label>
                                <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $commission->payment_date ? $commission->payment_date->format('Y-m-d') : '') }}" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            </div>
                            
                            <div class="payment-details" id="bank-account-details" style="{{ $commission->status == 'paid' ? 'display: block;' : 'display: none;' }}">
                                ...
                                <select id="bank_account_id" name="bank_account_id" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach($bankAccounts as $account)
                                        <option value="{{ $account->id }}" {{ (old('bank_account_id', $commission->transaction->bank_account_id ?? null) == $account->id) ? 'selected' : '' }}>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('notes', $commission->notes) }}</textarea>
                        </div>
